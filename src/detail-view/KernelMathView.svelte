<script>
  ...
  const redrawDetailedConvViewLegend = (arg) => {
    ...
    let normalizedColor = (v) =>
      colorScale(v * (1 - 2 * gradientGap) + gradientGap);
    ...
    let detailedLegend = detailedViewKernel
      .append("g")
      .attr("id", `detailed-legend-0`);

    let legendGroup = detailedLegend
      .append("g")
  ...
  const redraw = () => {
    ...
    columns
      ...
      .style("fill", function (d) {
        let normalizedValue =
          (kernel[d.row][d.col].text + kernelRange.range / 2) /
          kernelRange.range;
        const gap = 0.2;
    ...
    texts
      ...
      .style(
        "font-size",
        Math.floor(constraint / (textConstraintDivisor - 1)) + "px",
      )
      .attr("x", function (d) {
        return d.x === 1
...
<div class="legend" bind:this={legendFinal}>
  <!-- <svg id="legend" width=100% height=100%></svg> -->
</div>

<div class="grid" bind:this={gridFinal}>
